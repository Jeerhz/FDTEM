...
plots.py — figures from the analysis CSVs + tidy dataframe.
  ...
  dilution_<metric>.png       Δ(m) per k (incl. k=1 = undiluted severity effect),
                              faceted severity × family (null model)
  clean_curve_<metric>.png    error-free prefix score vs k (concatenation bias)
...
def make_plots(cfg: dict, df: pd.DataFrame) -> None:
    ...
    clean = pd.read_csv(ad / "clean_curve.csv")
    ...
    for metric in sorted(df["metric"].unique()):
        ...
        ks_all = sorted(int(k) for k in dm["k"].unique())

        # ── observed vs predicted (best candidate per k ≥ 2) ───────────────
        if ks:
        ...
        if len(dmm):
            ...
            fig.suptitle(f"{metric}: dilution curve Δ(m, k, severity, family)\n"
                         "(k=1 = undiluted severity effect size)", y=1.03)
            _save(fig, fd / f"dilution_{metric}.png")

        # ── clean curve: error-free prefix score vs k ──────────────────────
        cl = clean[clean["metric"] == metric].sort_values("k")
        if len(cl):
            fig, ax = plt.subplots(figsize=(5.5, 4))
            ax.errorbar(cl["k"], cl["mean_S"],
                        yerr=[cl["mean_S"] - cl["ci_lo"], cl["ci_hi"] - cl["mean_S"]],
                        marker="o", capsize=4, color="tab:green")
            ax.set_xlabel("k (sentences per paragraph)")
            ax.set_ylabel("mean score of error-free prefix")
            ax.set_title(f"{metric}: concatenation curve on error-free text\n"
                         "(same families at every k — pure length response)")
            ax.grid(alpha=0.3)
            _save(fig, fd / f"clean_curve_{metric}.png")
