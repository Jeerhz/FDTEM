#!/usr/bin/env python3
"""
data.py — aligned sentence units and k-sentence blocks.
...
Only contiguous same-document units are ever concatenated (coherent discourse,
single domain); blocks are non-overlapping windows of k units.
"""
...
class Block:
    ...
    k: int
    block_idx: int            # index within (lang, k) — drives counterbalancing
    doc_id: str
    ...
    def block_id(self) -> str:
        return f"{self.lang}_k{self.k}_{self.doc_id}_{self.start}"

    def join_src(self) -> str:
        return " ".join(self.src_units)          # source is always English here

    def join_tgt(self, units: Optional[List[str]] = None) -> str:
        return joiner_for(self.lang).join(units if units is not None else self.tgt_units)
...
# Blocks
# ════════════════════════════════════════════════════════════════════════════
def build_blocks(cfg: dict, docs_by_lang: Dict[str, List[Doc]]) -> List[Block]:
    """Non-overlapping contiguous k-unit windows, token-filtered, capped per (lang,k)."""
    des = cfg["design"]
        ...
        for k in des["k_list"]:
            ...
            for doc in docs:
                ...
                for start in range(0, n - k + 1, k):
                    cand.append(Block(lang=lang, lp=lp, k=k, block_idx=-1,
                                      doc_id=doc.doc_id, start=start,
                                      src_units=doc.src_units[start:start + k],
                                      tgt_units=doc.tgt_units[start:start + k]))
            rng.shuffle(cand)
            kept: List[Block] = []
            for b in cand:
                if len(kept) >= des["n_blocks_per_k"]:
                    break
                b.n_tokens_src = counter.count(b.join_src())
                b.n_tokens_tgt = counter.count(b.join_tgt())
                b.truncated = max(b.n_tokens_src, b.n_tokens_tgt) > des["max_tokens"]
                if b.truncated and des.get("drop_truncated", True):
                    continue
                kept.append(b)
            for i, b in enumerate(kept):
                b.block_idx = i
            blocks.extend(kept)
            logger.info(f"  blocks[{lang}, k={k}]: {len(kept)} kept "
