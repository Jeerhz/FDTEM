...
def score_jobs(cfg: dict, mcfg: dict, jobs: List[dict]) -> Dict[str, float]:
    ...
    Returns {job_key: score}. Note the k=1 paragraph and its single sentence
    share one cache key (identical src/mt/ref), so S ≡ s_1 at k=1 by
    construction — the trivial anchor of the design.
    """
    name = mcfg["name"]
