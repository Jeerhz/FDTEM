...
Pipeline: print plan -> load docs -> build nested k_max families -> perturb each
unit once per condition (cached) -> assemble monotone perturbation chains ->
score sentences + prefix paragraphs (cached) -> tidy dataframe -> fit candidate
aggregation functions -> controls -> plots -> SUMMARY.md with one verdict
sentence per metric.
"""
...
def print_plan(cfg: dict, metrics: list) -> None:
    ...
    print("  Design: nested paired families — every scale is a prefix of the same")
    print("    k_max window; per-unit perturbations generated once and frozen; the")
    print("    perturbation set grows as a monotone chain (new errors only arrive")
    print("    with new segments). Length is the only variable across scales.")
    print(f"\n  Data        : {cfg['data']['source']}  (resegment={cfg['data'].get('resegment')})")
    print(f"  Scales      : k={k_list} (k_max={max(k_list)}), "
          f"{cfg['design']['n_families']} families per lang")
    print(f"  Conditions  : {all_conditions(cfg)}  × m=0..k (chain), one edit per segment")
    ...
    print(f"  Fit         : affine={cfg['fit']['affine']}, CV folds={cfg['fit']['cv_folds']} (by doc), "
          f"bootstrap n={cfg['fit']['bootstrap']['n_resamples']}; k=1 excluded from fits")
    print(f"  Token limit : {cfg['design']['max_tokens']} on the k_max prefix "
          f"(drop={cfg['design']['drop_truncated']})")
    print("\n  Environment:")
...
def write_summary(cfg: dict, summaries: dict, out: Path) -> None:
    ...
    lines += [
        "**Protocol.** Nested paired families of contiguous WMT24++ post-edit "
        "segments: every scale k is a prefix of the same k_max window; each unit's "
        "perturbation (severity × error-family, one edit per segment, "
        "length-preserving) is generated once and frozen at all scales; perturbation "
        "sets grow as monotone chains, so m ∈ {0..k} is covered exactly once per "
        "(family, condition, k) with counterbalanced positions. s_i = score of "
        "sentence i alone, S = score of the concatenated prefix; candidates fitted "
        "with affine calibration and compared by document-grouped CV R² (k ≥ 2).",
        "", "## Verdicts", ""]
...
def main():
    ...
    if args.smoke:
        ...
        cfg["design"]["n_families"] = sm["n_families"]
    ...
    banner("1/5 Data")
    from data import build_families, load_docs
    docs = load_docs(cfg)
    families = build_families(cfg, docs)
    ...
    items = build_items(cfg, families, perturber)
    perturber.flush()
