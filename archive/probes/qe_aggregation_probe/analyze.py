...
----------------------
  candidate_fits.csv        all candidates × (metric, k≥2): R², CV-R², AIC/BIC, p̂+CI
  ...
  residual_by_k.csv         S − f̂(parts) with f frozen at the smallest k ≥ 2
  dilution.csv              Δ(m, k, severity, family) — the null-model curve
                            (includes k=1: the undiluted severity effect size)
  clean_curve.csv           mean S of error-free prefixes vs k (Zhang et al.-style
                            concatenation bias on our items)
  mixed_effects_<metric>.txt  optional MixedLM on residuals

k=1 cells are excluded from aggregation fitting: S ≡ s_1 there by construction
(identical scoring input), so every candidate is the identity — but k=1 rows
anchor the dilution and clean-curve tables.
"""
...
from fit import (FitResult, best_by_cv, fit_cell, fit_candidate,
...
def _doc_clustered_ci(values: np.ndarray, docs: np.ndarray):
    """Mean ± 1.96·SEM over document-level means (clusters)."""
    s = pd.Series(values).groupby(pd.Series(docs)).mean()
    m = float(s.mean())
...
def analyze(cfg: dict, df: pd.DataFrame) -> Dict[str, dict]:
    ...
    pos_rows, frozen_rows, clean_rows = [], [], []
    summaries: Dict[str, dict] = {}

    for metric in sorted(df["metric"].unique()):
        dm = df[df["metric"] == metric].reset_index(drop=True)
        kind = kind_of[metric]
        ks_fit = sorted(int(k) for k in dm["k"].unique() if k >= 2)
        best_cells: Dict[int, FitResult] = {}
        cell_data: Dict[int, tuple] = {}

        # ── clean curve: error-free prefix score vs k (all scales incl. 1) ──
        for k in sorted(dm["k"].unique()):
            d0 = dm[(dm["k"] == k) & (dm["m"] == 0)]
            if len(d0):
                m, lo, hi, ndoc = _doc_clustered_ci(d0["S"].to_numpy(float),
                                                    d0["doc_id"].to_numpy())
                clean_rows.append({"metric": metric, "k": int(k), "mean_S": m,
                                   "ci_lo": lo, "ci_hi": hi, "n_docs": ndoc,
                                   "n": len(d0)})

        # ── per-k fits, all candidates (k ≥ 2 only) ────────────────────────
        for k in ks_fit:
            ...
            # position (symmetry) control on m=1 items
            m1 = dk["m"] == 1
            if m1.sum() >= 10:
        ...
        # ── pooled fit across k ≥ 2 (mean vs sum separate here) ────────────
        dp = dm[dm["k"] >= 2].reset_index(drop=True)
        pooled = fit_cell(parts_of(dp, kind, eps), dp["S"].to_numpy(float),
                          dp["doc_id"].to_numpy(), kind, fit_cfg, seed,
                          bootstrap=False)
        ...
        for k in ks_fit:
            ...
            frozen_rows.append({"metric": metric, "k": k, "frozen_at_k": k_min,
                                ...
                                "ci_lo": lo, "ci_hi": hi, "n_docs": ndoc,
                                "n": len(y)})
            if k == max(ks_fit) and cfg["fit"].get("mixed_effects", False):
    ...
    # block_id = family_id_k, so S0 is the clean prefix of the SAME family at
    # the SAME scale; k=1 rows give the undiluted severity effect size.
    ...
    pd.DataFrame(clean_rows).to_csv(ad / "clean_curve.csv", index=False)
