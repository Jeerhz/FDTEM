...
Evaluate a plain XLM-RoBERTa encoder (no COMET fine-tuning) on XGlue tasks
via a frozen linear probe. This serves as the baseline to validate that the
evaluation code is correct before comparing with COMET-fine-tuned encoders.
...
---------------
  ...
  nc     : News Classification — train on English, 0-shot transfer to 4 languages
             (requires --xglue_tar path/to/xglue_full_dataset.tar.gz)

Sentence embeddings
-------------------
Average pooling over all non-[CLS]/non-[SEP]/non-[PAD] token embeddings from
the last hidden layer of XLM-R. This is a standard strong baseline for
cross-lingual transfer with frozen encoders.

Usage
-----
  # XLM-R base (to check the code is correct, expect ~70% on XNLI)
  python scripts/eval_xlmr_xglue.py \\
      --model xlm-roberta-base \\
      --xglue_tar data/xglue_full_dataset.tar.gz \\
      --wandb_project comet-xglue

  # XLM-R large (stronger baseline, expect ~80% on XNLI)
  python scripts/eval_xlmr_xglue.py \\
      --model xlm-roberta-large \\
      --xglue_tar data/xglue_full_dataset.tar.gz \\
      --output results/xglue_xlmr_large.json \\
      --wandb_project comet-xglue
"""
import argparse
import io
...
def load_xlmr(model_name: str, device: str):
    ...
    logger.info(f"Model loaded  ({n_params:.1f}M params, device={device})")
...
def encode_texts(
    ...
    """Encode texts with XLM-R using mean pooling over non-special tokens."""
    ...
    for start in tqdm(range(0, len(texts), batch_size), desc=f"  {desc}", leave=False):
        ...
        # Mask out CLS, SEP, and PAD tokens
        special_mask = (
            (input_ids != pad_id) &
            (input_ids != cls_id) &
            (input_ids != sep_id)
        ...
        pooled = (last_hidden * special_mask).sum(dim=1) / special_mask.sum(dim=1).clamp(min=1e-9)
...
def eval_xnli(tokenizer, model, batch_size, device, languages, max_train, wandb_run=None):
    ...
    u_tr = encode_texts(tokenizer, model, list(train_ds["premise"]),    batch_size, device, "premise train")
    v_tr = encode_texts(tokenizer, model, list(train_ds["hypothesis"]), batch_size, device, "hypo  train")
    ...
    for lang in languages:
        ...
        try:
            ...
            continue
        u_te = encode_texts(tokenizer, model, list(test_ds["premise"]),    batch_size, device, "premise test")
        v_te = encode_texts(tokenizer, model, list(test_ds["hypothesis"]), batch_size, device, "hypo  test")
...
def eval_paws_x(tokenizer, model, batch_size, device, languages, max_train, wandb_run=None):
    from datasets import load_dataset
    ...
    logger.info(f"  Training set: {len(train_ds)} English examples")
    u_tr = encode_texts(tokenizer, model, list(train_ds["sentence1"]), batch_size, device, "s1 train")
    v_tr = encode_texts(tokenizer, model, list(train_ds["sentence2"]), batch_size, device, "s2 train")
    X_train = pair_features(u_tr, v_tr)
    y_train = np.array(train_ds["label"])
    ...
    for lang in languages:
        ...
        try:
            ...
            logger.warning(f"    Skipping — could not load {lang}: {exc}")
            continue
        u_te = encode_texts(tokenizer, model, list(test_ds["sentence1"]), batch_size, device, "s1 test")
        v_te = encode_texts(tokenizer, model, list(test_ds["sentence2"]), batch_size, device, "s2 test")
        X_test = pair_features(u_te, v_te)
        y_test = np.array(test_ds["label"])
        m = linear_probe(X_train, y_train, X_test, y_test)
        results[lang] = m
...
def eval_nc(tokenizer, model, batch_size, device, languages, max_train, xglue_tar, wandb_run=None):
    logger.info("── NC (News Classification) ─────────────────────────")

    if not xglue_tar or not os.path.isfile(xglue_tar):
        logger.warning("  NC skipped: pass --xglue_tar path/to/xglue_full_dataset.tar.gz")
        return {}
    ...
    with tarfile.open(xglue_tar, "r:gz") as tar:
        ...
        X_train = encode_texts(tokenizer, model, train_texts, batch_size, device, "NC train")
        ...
        for lang in languages:
            ...
            if not valid:
                ...
                continue
            test_texts_f, test_raw_f = zip(*valid)
            X_test = encode_texts(tokenizer, model, list(test_texts_f), batch_size, device, f"NC test {lang}")
            y_test = np.array([label2id[lbl] for lbl in test_raw_f])
            m = linear_probe(X_train, y_train, X_test, y_test)
            results[lang] = m
            logger.info(f"    acc={m['accuracy']:.4f}  f1={m['f1_macro']:.4f}")
            if wandb_run is not None:
...
def plot_task(task: str, task_res: Dict, out_dir: Path, run_name: str) -> None:
    ...
    langs = [lang for lang in task_res if lang != "avg"]
    accs  = [task_res[lang]["accuracy"] for lang in langs]
    f1s   = [task_res[lang]["f1_macro"] for lang in langs]
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output", default="results/xglue_xlmr.json")
    p.add_argument("--xglue_tar", default=None,
                   help="Path to xglue_full_dataset.tar.gz (required for NC).")
...
def main() -> None:
    ...
    tokenizer, model = load_xlmr(args.model, device)

    run_name = args.run_name or args.model.split("/")[-1]
    ...
    results: Dict = {
        ...
        "embedding": "mean_pool_last_layer",
        "tasks": {},
    }
    ...
    task_fns = {
        "xnli":   lambda: eval_xnli(tokenizer, model, args.batch_size, device,
                                     args.languages or XNLI_LANGS, args.max_train_samples, wandb_run),
        "paws-x": lambda: eval_paws_x(tokenizer, model, args.batch_size, device,
                                       args.languages or PAWS_LANGS, args.max_train_samples, wandb_run),
        "nc":     lambda: eval_nc(tokenizer, model, args.batch_size, device,
